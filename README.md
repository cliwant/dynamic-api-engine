...
모든 `/api/*` 경로로 들어오는 요청은 **Universal Router**가 받습니다. 하드코딩된 엔드포인트가 아니라, DB에서 동적으로 라우팅합니다.
...
4. SQL Injection 패턴 검사 → "'; DROP TABLE" 감지 시 차단
```
...
새 API를 만들려면 코드 변경 없이 **DB에 행만 추가**하면 됩니다.
...
API 수정이 필요할 때는 기존 버전을 수정하지 않고 **새 버전을 추가**합니다.
...
- 모든 버전 변경은 `APP_API_AUDIT_H`에 자동 기록
